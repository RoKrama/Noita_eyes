...
constexpr std::array<int, 99> one {50, 66, 05, 48, 62, 13, 75, 29, 24, 61, 42, 70, 66, 62, 32, 14, 81, 8, 15, 78, 02, 29, 13, 49, 01,
...
constexpr std::array < int, 103> two {80, 66, 05, 48, 62, 13, 75, 29, 24, 61, 42, 70, 66, 62, 32, 14, 81, 8, 15, 78, 02, 29, 13, 49, 01,
...
constexpr std::array< int, 118> three {36, 66, 05, 48, 62, 13, 75, 29, 24, 61, 42, 70, 66, 62, 32, 14, 81, 8, 15, 78, 02, 29, 13, 49, 01,
...
constexpr std::array <int, 102> four {76, 66, 05, 49, 75, 54, 69, 46, 32, 01, 42, 60, 26, 48, 50, 80, 32, 24, 55, 61, 47, 12, 21, 12, 49,
...
constexpr std::array <int, 137> five {63, 66, 05, 49, 75, 54, 02, 60, 29, 40, 78, 47, 60, 75, 67, 71, 60, 02, 65, 07, 47, 14, 45, 74, 59,
...
constexpr std::array <int, 124> six {34, 66, 05, 49, 75, 54, 23, 74, 11, 13, 28, 26, 19, 48, 67, 57, 37, 60, 34, 28, 74, 10, 17, 32, 11,
...
constexpr std::array <int, 119> seven {27, 66, 05, 49, 75, 54, 02, 60, 29, 40, 02, 55, 9, 15, 59, 18, 68, 03, 36, 05, 47, 77, 44, 38, 01,
...
constexpr std::array <int, 120> eight {77, 66, 05, 49, 75, 54, 02, 60, 29, 40, 02, 55, 9, 15, 59, 18, 68, 03, 36, 05, 47, 60, 21, 80, 01,
...
constexpr std::array <int, 114> nine {33, 66, 05, 49, 75, 54, 02, 60, 29, 40, 02, 55, 9, 15, 59, 18, 68, 03, 36, 05, 47, 33, 21, 59, 44,
...
template<int N> 
void compute( const std::array<int, N>& cypher, int target, std::array<std::array<int, 83>, 83>& counter)
{
    int found{ 0 };
    std::array<int, 83>& counterPlace{counter[target]};
    for (int i = 0; i < N; i++)
    {
        bool found{ false };
        if (!found)
...
int main()
{
        std::array<std::array<int, 83>,83> freq;
        ...
        for (int i = 0; i < 83; i++)
        {
            compute(one, i, freq);
            compute(two, i, freq);
            compute(three, i, freq);
            compute(four, i, freq);            
            compute(five, i, freq);
            compute(six, i, freq);
            compute(seven, i, freq);
            compute(eight, i, freq);            
            compute(nine, i, freq);
        }

        std::ofstream ostream;
        ostream.open("output_eyes.csv");

        for (int i = 0; i < 83; i++)
        {

            for (int c = 0; c < 83; c++)
                ostream << " | " << static_cast<char>((i + c) % 83 + 32) << " | " << freq[i][c] << " | ";
            ostream << std::endl;
        }
